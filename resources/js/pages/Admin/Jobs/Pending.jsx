 ...
 *         ],
 *     ]);
 * })->middleware(['auth', 'verified'])->name('admin.jobs.pending');
 * 
 * NOTE: This is a view-only page using mock data, no API calls are made.
 */

export default function PendingJobs({ auth, jobs = null, meta = null, filters = {} }) {
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        console.log({ q: searchQuery, perusahaan: perusahaanFilter });
    ...
    const handleApproveJob = (id, judul) => {
        if (window.confirm(`Apakah Anda yakin ingin menyetujui lowongan "${judul}"?`)) {
            console.log({ action: 'approve', id_lowongan: id });
            // In a real application, this would send a request to the server
        }
    };

    const handleRejectJob = (id, judul) => {
        if (window.confirm(`Apakah Anda yakin ingin menolak lowongan "${judul}"?`)) {
            console.log({ action: 'reject', id_lowongan: id });
            // In a real application, this would send a request to the server
        }
    };
        ...
        <AuthenticatedLayout
            header={
                <div className="flex items-center justify-between">
                    <h2 className="font-semibold text-xl text-gray-800 leading-tight">Admin - Pending Jobs</h2>
                    <div>
                        <Link
                        ...
                        <div className="p-6">
                            ...
                            {displayJobs.length > 0 ? (
                                                ...
                                                <tr key={job.id_lowongan}>
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                                                        <div className="flex space-x-2">
                                                            ...
                                                            <button
                                                                onClick={() => handleApproveJob(job.id_lowongan, job.judul)}
                                                                className="bg-green-500 hover:bg-green-600 px-2 py-1 rounded text-xs font-medium text-white"
                                                                aria-label={`Approve lowongan ${job.judul}`}
                                                            >
                                                                Approve
                                                            </button>
                                                            
                                                            <button
                                                                onClick={() => handleRejectJob(job.id_lowongan, job.judul)}
                                                                className="bg-red-500 hover:bg-red-600 px-2 py-1 rounded text-xs font-medium text-white"
                                                                aria-label={`Tolak lowongan ${job.judul}`}
                                                            >
                                                                Tolak
                                                            </button>
                                                        </div>
                                ...
                                <div className="text-center py-8">
                                    <p className="text-lg font-medium text-gray-700">Tidak ada lowongan yang perlu ditinjau</p>
                                    <p className="text-sm text-gray-500">Semua lowongan telah diproses.</p>
                                </div>
                            )}
                                ...
                                <div className="flex items-center justify-between mt-4">
                                    <div>
                                        <p className="text-sm text-gray-700">
                                            Menampilkan {displayMeta.page} dari {Math.ceil(displayMeta.total / displayMeta.perPage)} halaman
                                        </p>
                                    </div>
                                    <div className="flex space-x-2">
                                        <button
                                        ...
                                        <button
                                            ...
                                            disabled={displayMeta.page >= Math.ceil(displayMeta.total / displayMeta.perPage)}
                                            className={`px-4 py-2 text-sm font-medium rounded-md
                                                ${displayMeta.page >= Math.ceil(displayMeta.total / displayMeta.perPage)
                                                    ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                                                    : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}
                                            `}
                                        >
